...
- 新增像素小熊儿童听音选图游戏页：每题播放英文单词发音（合成语音以 G.711
  u-law 打进固件），屏上三张 Twemoji 图卡对应三个物理按键；选对加 1 分并自动
  下一题，选错闪红、重放发音并提示再试一次。积分与题目游标通过 NVS 持久化，
  长按上键重听。词库（120 个常见具象词）在 `assets/words/words.tsv`，
  `tools/words_pack.py` 重新生成词表、配图、发音与 16px 中文字体。
- 新增像素风 Todo 清单应用页：上/下键导航，确定键勾选，长按上键从模板池新增任务，
  长按下键删除，带进度条、滚动指示、电量显示，清单通过 NVS 持久化，重启不丢失。
