...
- 新增像素小熊儿童看图单词应用页：120 个常见具象词按 10 词一组练习，每词配
  Twemoji 图片；上/下键切词，确定键翻看/收起中文，组末庆祝、组内进度点、
  电量显示，复习游标通过 NVS 持久化。词库在 `assets/words/words.tsv`，
  `tools/words_pack.py` 重新生成词表、配图与 16px 中文字体。
